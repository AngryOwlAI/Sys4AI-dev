...
class TraceSemanticTests(unittest.TestCase):
    def test_live_semantics_pass_with_tx24_review_evidence(self) -> None:
        ...
        self.assertEqual(157, sum(row["verification_status"] == "planned" for row in rows))
    ...
    def test_post_tx31_state_requires_controlled_next_transaction_route(self) -> None:
        def mutate_state(state):
    ...
    def test_policy_detects_silent_planned_gap_change(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            policy.write_text(
                ...
                "expected_planned_verification = 157\n"
        ...
        self.assertFalse(result.ok)
        self.assertTrue(any("expected planned=157, found 156" in item for item in result.messages))
